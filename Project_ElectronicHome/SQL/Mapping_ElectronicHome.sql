...
\dt tool.*
/*LEEME*/
/*aunque unos incluyan letras, comohay un correlativo, lo mejor sería guardar el dato, luego pasarlo a string y addle el extra..*/
...
/*ALTER SEQUENCE humanResourcesControl.Contract.ID RESTART WITH 562355;
    NOTA: es que no cree una secuencia, pero no se si al hacer eso podría seguirlo manejando como serial, así que mejor que empiece desde 1... ;-;*/
...
\dt humanResourcesControl.*/*to see every single table that are involve in this schema/scope*/
...
CREATE TABLE goodsControl.Appliance(
    ...
    theBrand goodsControl.brand NOT NULL,/*check*/
...
CREATE TABLE goodsControl.Stock(
    ...
    FOREIGN KEY(product) REFERENCES goodsControl.Product(code) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE transactionControl.Transfer(
    ...
    transferState transactionControl.state DEFAULT 'pending' NOT NULL,/*ira cb según la interacción con la interfaz... solo done, tendrá botón para ser marcado...*/
...
CREATE TABLE transactionControl.Transferred(
    ...
    requested INT2 CHECK(requested >= 1) DEFAULT 1,
    sended INT2 CHECK(sended >= 0),/*esto se llenará hasta que procese la solicitud*/
...
DROP TABLE transactionControl.Sold;
...
/*Para hacer algo como en mysql cuando se quiere setear el número ini de AUTOINCREMENTABL
  (solo que porque no sabía si e dato seguriía siendo SERIAL o tendría que tratarlo como 
  el objeto SEQUENCE así coo se creó (lo cual no me convenía) decidí, no applo*/
